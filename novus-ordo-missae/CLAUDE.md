...
- **Responsorial psalms are the exception**: no `body` slot. Instead, `responsorialPsalm.responsory.primary.<lang>` is a `Line[]` (the refrain), `responsorialPsalm.responsory.alternatives[]` is an array of `{<lang>: Line[]}` for `vel`/`Or:` alt refrains, and `responsorialPsalm.verses.<lang>` is a `Line[][]` (per-lang verse list, each verse a `Line[]`). Verse counts may differ per language because the source typesets stanza groupings independently per lang. The split runs as the last step in `_post_process_mass` (`_split_responsorial_psalms_in_mass`); body-shape passes still see the legacy `body.lines` shape.
- **Gospel acclamations are also typed, not Prayer-shaped**: no `body` slot. `gospelAcclamation.mode` is a season discriminator — `alleluia` (refrain present, default), `versus-ante-evangelium` (Lent, no refrain), or `alleluia-or-versus` (fixed-date feasts that can fall in either season; refrain marked optional with parens). `acclamation.<lang>` is `Line[]` carrying the people's refrain as `{type:'response'}` segments, `verse.<lang>` is `Line[]` for the cantor's scripture (with response segments at the boundaries when the source wraps it in `Allelúia.`), and `citation.<lang>` is the scripture reference. `acclamation` is omitted on pure-Lent slots. Multi-option slots wrap `alternatives: [...]` (All Souls / Sacred Heart / Holy Family). Parens-conditional `(Allelúia.)` preserves the parens literally inside the response segments — consumers can render the optional refrain conditionally for Lent. The printed per-lang header (Alleluia / Versus ante Evangelium / Canto al Vangelo / …) is intentionally NOT stored: it's either redundant with the refrain (la/es/fr) or constant per language (pt-BR/it/de) — UI strings, not data. Mode is derived from the la/es/fr label during the split. The split runs after the responsorialPsalm split as the final step of `_post_process_mass` (`_split_gospel_acclamation_in_mass`).
